...
export const Form = {
    ...
    renderInput(label: string, id: string, type: string = "text", value: string = "", placeholder: string = ""): string {
        return `
    ...
    renderTextarea(label: string, id: string, value: string = "", rows: number = 4, placeholder: string = ""): string {
        return `
            <label style="display:block; margin-bottom:5px; font-size:0.8rem; color:#aaa;">${label}</label>
    ...
    renderSelect(label: string, id: string, options: { value: string, label: string }[], selectedValue: string = ""): string {
        return `
            <label style="display:block; margin-bottom:5px; font-size:0.8rem; color:#aaa;">${label}</label>
            <select id="${id}" style="width: 100%; padding: 10px; background: #252525; border: 1px solid #3a3a3a; border-radius: 6px; color: #e0e0e0; margin-bottom: 15px;">
                ${options.map((opt: { value: string, label: string }): string => `
    ...
    getData(ids: string[]): Record<string, string> {
    ...
    showError(id: string, message: string): void {
        const errEl: HTMLElement | null = document.getElementById(`err-${id}`);
        const inputEl: HTMLElement | null = document.getElementById(id);
        if (errEl) {
