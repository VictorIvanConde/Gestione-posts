...
export const CommentsPage = {
    async render(container, state) {
        ...
        let comments = allComments.filter((c) => c.isActive !== false);
        ...
        let html = Header.render("Gestione COMMENTI", true, state.ricerca);
    ...
    openCreate() {
        ...
        document.getElementById("saveCreate")?.addEventListener("click", async () => {
            const data = Form.getData(["createPostId", "createName", "createEmail", "createBody"]);
            // controllo rapido sulla compilazione dei campi
            if (!data.createEmail || !data.createBody || !data.createName)
