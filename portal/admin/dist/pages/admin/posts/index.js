...
export const PostsPage = {
    async render(container, state) {
        ...
        let posts = allPosts.filter((p) => p.isActive !== false);
        ...
        let html = Header.render("Gestione POST", true, state.ricerca);
    ...
    openCreate() {
        ...
        document.getElementById("saveCreate")?.addEventListener("click", async () => {
            const fieldIds = ["createId", "createTitle", "createBody", "createUserId"];
            Form.clearErrors(fieldIds);
            const data = Form.getData(fieldIds);
            // verifico che i campi obbligatori siano compilati
            if (!data.createTitle)
                return Form.showError("createTitle", "Il titolo è obbligatorio");
            if (!data.createBody)
                return Form.showError("createBody", "Il contenuto è obbligatorio");
            if (!data.createUserId)
                return Form.showError("createUserId", "L'ID utente è obbligatorio");
            // controllo se l'id esiste già nel sistema
            if (data.createId) {
                try {
                    const existing = await PostsService.getById(data.createId);
                    if (existing) {
